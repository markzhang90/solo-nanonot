...
def get_workspace_path(workspace: str | Path | None = None) -> Path:
    ...
    path = Path(workspace).expanduser() if workspace else Path.home() / ".nanobot_new" / "workspace"
    return ensure_dir(path)


def is_default_workspace(workspace: str | Path | None) -> bool:
    """Return whether a workspace resolves to nanobot's default workspace path."""
    current = Path(workspace).expanduser() if workspace is not None else Path.home() / ".nanobot_new" / "workspace"
    default = Path.home() / ".nanobot_new" / "workspace"
    return current.resolve(strict=False) == default.resolve(strict=False)


def get_cli_history_path() -> Path:
    """Return the shared CLI history file path."""
    return Path.home() / ".nanobot_new" / "history" / "cli_history"
