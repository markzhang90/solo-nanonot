    ...
    *,
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if (
            ...
            and metadata.get("type") == "reminder_todos_refresh"
        ):
            from nanobot.apps.reminder.app import ReminderApp
            from nanobot.apps.reminder.legacy import refresh_today_todos_from_service

            reminder_app = ReminderApp.from_config(config, config.workspace_path, cron)
            if not reminder_app.service_enabled:
                return "reminder todos refresh skipped: service disabled"
            result = await refresh_today_todos_from_service(
                reminder_app.workspace,
                reminder_app.tool_config(),
                user_cron=reminder_app.scheduler,
                mode="sync_only",
            )
            if result.get("ok") is not True:
                err = result.get("error") if isinstance(result.get("error"), dict) else {}
                message = str(err.get("message") or "Reminder Service sync failed.")
                raise RuntimeError(message)
            data = result.get("data") if isinstance(result.get("data"), dict) else {}
            return (
                "reminder todos refresh ok: "
                f"merged={data.get('merged_count', 0)} tasks={data.get('tasks_count', 0)}"
            )
        ...
        metadata = job.payload.origin_metadata or {}
        if is_bound_cron_job(job) and metadata.get("type") == "task_wakeup":
            from nanobot.apps.reminder.legacy import prepare_task_wakeup_from_cron

            prepared = prepare_task_wakeup_from_cron(
                config.workspace_path,
                {**metadata, "_cron_job_id": job.id},
            )
            data = prepared.get("data") if isinstance(prepared.get("data"), dict) else {}
            if data.get("action") == "skip":
                return str(data.get("summary") or "task wakeup skipped")
            content = str(data.get("content") or "").strip()
            if content:
                job.payload.message = content
