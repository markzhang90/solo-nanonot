...
def test_default_config_path_uses_new_instance_dir(monkeypatch) -> None:
    monkeypatch.setattr("nanobot.config.loader._current_config_path", None)
    assert get_config_path() == Path.home() / ".nanobot_new" / "config.json"
...
def test_shared_and_legacy_paths_remain_global() -> None:
    assert get_cli_history_path() == Path.home() / ".nanobot_new" / "history" / "cli_history"
    assert get_legacy_sessions_dir() == Path.home() / ".nanobot" / "sessions"


def test_workspace_path_is_explicitly_resolved() -> None:
    assert get_workspace_path() == Path.home() / ".nanobot_new" / "workspace"
...
def test_is_default_workspace_distinguishes_default_and_custom_paths() -> None:
    ...
    assert is_default_workspace(Path.home() / ".nanobot_new" / "workspace") is True
