...
Periodic background checks are driven by `HEARTBEAT.md` in your workspace (`~/.nanobot_new/workspace/HEARTBEAT.md`). When `nanobot gateway` starts, it registers a protected heartbeat cron job by default. Every 30 minutes, that job checks the file; if it finds tasks under `## Active Tasks`, the agent executes them and delivers only results that pass the notification gate to your most recently active chat channel. If there are no active tasks, or the result is routine with nothing useful to report, the heartbeat is skipped silently.
...
**Setup:** edit `~/.nanobot_new/workspace/HEARTBEAT.md` (created automatically by `nanobot onboard`):
...
You can change the interval or disable the built-in heartbeat in `~/.nanobot_new/config.json`:
