...
Most examples below are snippets to merge into `~/.nanobot_new/config.json`. When a snippet includes `allowFrom`, it is showing a static allowlist. For pairing-based access on supported channels, omit `allowFrom`; Slack and Mattermost also need `dm.policy` set to `"allowlist"` for DMs to issue pairing codes.
...
3. Merge that platform's JSON snippet into `~/.nanobot_new/config.json`.
...
nanobot will automatically register, configure `~/.nanobot_new/config.json`, and connect to Mochat.
...
If you prefer to configure manually, add the following to `~/.nanobot_new/config.json`:
...
```json
{
  "channels": {
    "whatsapp": {
      "databasePath": "~/.nanobot_new/whatsapp-auth/neonize.db"
    }
  }
}
```
