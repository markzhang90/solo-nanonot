...
`Nanobot.from_config()` reuses your normal `~/.nanobot_new/config.json` and
`~/.nanobot_new/workspace/`. Provider, model, tools, memory, and session behavior
...
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.nanobot_new/config.json`. |
