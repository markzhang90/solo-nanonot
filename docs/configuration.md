...
Config file: `~/.nanobot_new/config.json`
...
`~/.nanobot_new/auth/xai.json`), separately from Grok Build so rotating refresh
...
**2. Set model** (merge into `~/.nanobot_new/config.json`):
```json
{
  "modelPresets": {
    "copilot": {
...
**2. Add to config** (partial — merge into `~/.nanobot_new/config.json`):
```json
{
  "providers": {
    "ollama": {
...
**3. Add to config** (partial — merge into `~/.nanobot_new/config.json`):
...
Global settings that apply to all channels. Configure under the `channels` section in `~/.nanobot_new/config.json`:
...
nanobot incorporates basic tools for accessing the web. These include searching via APIs, and fetching arbitrary web pages in Markdown format. They are enabled by default, and can be configured in `~/.nanobot_new/config.json` under `tools.web`.
...
nanobot supports multiple web search providers. Configure in `~/.nanobot_new/config.json` under `tools.web.search`.
