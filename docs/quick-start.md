...
| Path | Purpose |
|---|---|
| `~/.nanobot_new/config.json` | Provider, model, WebUI, channel, tool, and runtime settings |
| `~/.nanobot_new/workspace/` | Sessions, memory, skills, automations, and generated files |
...
Use this only when the wizard is unavailable or you intentionally manage JSON. First run `nanobot onboard`, then merge a provider and a named model preset into `~/.nanobot_new/config.json`.
